  ...
  Promise<void> {
    ...
    if (auxIdentifier.isAuxiliary && resourceId !== null) {
      ...
      if (resourceId === null) {
        ...
        // Update existing resource record
        const stringContent = await PostgresDataAccessor.streamToString(data);
        await this.database.queryHelper(`UPDATE ${this.schema}.resource_${podId} SET content = $1,
          updated_at = now() WHERE id = $2`, [ stringContent, resourceId ]);
      }
    }
