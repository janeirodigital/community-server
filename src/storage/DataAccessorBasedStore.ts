...
import type { CompositeAuxiliaryManager } from '../ldp/auxiliary/CompositeAuxiliaryManager';
...
export class DataAccessorBasedStore implements ResourceStore {
  ...
  private readonly auxiliaryManager: CompositeAuxiliaryManager;

  public constructor(accessor: DataAccessor, identifierStrategy: IdentifierStrategy,
    auxiliaryManager: CompositeAuxiliaryManager) {
  ...
  public async getRepresentation(identifier: ResourceIdentifier): Promise<Representation> {
    ...
    this.auxiliaryManager.addMetadata(identifier, metadata);
  ...
  public async deleteResource(identifier: ResourceIdentifier): Promise<void> {
    ...
    if (this.auxiliaryManager.isAuxiliaryIdentifier(identifier) && !this.auxiliaryManager.canDeleteRoot(identifier)) {
      const parentMetadata = await this.accessor.getMetadata(this.auxiliaryManager.getAssociatedIdentifier(identifier));
    ...
    // resources"
    // https://solid.github.io/specification/protocol#deleting-resources
    if (!this.auxiliaryManager.isAuxiliaryIdentifier(identifier)) {
      await this.safelyDeleteAuxiliaryResources(this.auxiliaryManager.getAuxiliaryIdentifiers(identifier));
    }
  ...
  protected async writeData(identifier: ResourceIdentifier, representation: Representation, isContainer: boolean,
    ...
    if (this.auxiliaryManager.isAuxiliaryIdentifier(identifier)) {
      await this.auxiliaryManager.validate(identifier, representation);
    }
  ...
  protected createSafeUri(container: ResourceIdentifier, metadata: RepresentationMetadata,
    ...
    // https://solid.github.io/specification/protocol#writing-resources
    if (this.auxiliaryManager.isAuxiliaryIdentifier(newID)) {
  ...
  protected getContainedAuxiliaryResources(metadata: RepresentationMetadata): NamedNode[] {
    return metadata.getAll(LDP.terms.contains).filter((object): boolean =>
      this.auxiliaryManager.isAuxiliaryIdentifier({ path: object.value })) as NamedNode[];
  }
