...
describe('A GreedyReadWriteLocker', (): void => {
  ...
  beforeEach(async(): Promise<void> => {
    ...
    locker = new GreedyReadWriteLocker(sourceLocker, store);
  });
  ...
  it('errors when trying to writeLock a count identifier.', async(): Promise<void> => {
    ...
    locker = new GreedyReadWriteLocker(sourceLocker, store, { count: 'dummy', read: 'read', write: 'write' });
    await expect(locker.withWriteLock({ path: `http://test.com/foo.dummy` }, (): any => 5))
      .rejects.toThrow(ForbiddenHttpError);
  });

  it('errors if there is a problem acquiring the lock counter.', async(): Promise<void> => {
    store.getRepresentation = jest.fn().mockRejectedValue(new Error('bad request!'));
    await expect(locker.withReadLock(resourceId, (): any => 5)).rejects.toThrow('bad request!');
  });
